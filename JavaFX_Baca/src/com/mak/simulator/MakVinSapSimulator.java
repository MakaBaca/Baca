...
public class MakVinSapSimulator extends AbstractBacaSimulator implements BacaSystemSimulator {
	...
	public static void main(String[] args) {
		...
		for(int i=1; i<=1; i++){
			MakVinSapSimulator sim = new MakVinSapSimulator();
			sim.prepareGame(78);
			sim.beginSimulation();
	...
	public MakVinSapSimulator(){
		System.out.println("1's\t2's\t3+\tOutcome\tVsays\tResult\tWagger\tScore\t#");
		System.out.println("===================================================================================");
	}
	...
	public int unitsToWagger() {
		if(totalWinings <= stopLoss){
			return 0;
		} else if(totalWinings <= fbStopLoss){
			fbStopLoss = fbStopLoss * 2;
			baseUnits++;
			return baseUnits;
		}else if(totalWinings < fbStopWin){
			return baseUnits;
		}else if(totalWinings >= fbStopWin){
			baseUnits = stopWin - baseUnits;
			return baseUnits;
		}else{
			return 0;
		}
		...
		while(game.hasNextHand()){
			...
			if(betOn != null){
				...
				if(waggerUnits > 0){
					...
					noOfHandsPlayed ++;
					System.out.print("\t"+waggerUnits+"\t"+totalWinings+"\t"+getTotalHands());
				}
			}
			System.out.println();
		}
